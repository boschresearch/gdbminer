...
void renderDocument(XMLDocument & xmlDocument, ImageBuffer & buffer)
{
  ...
  if (XMLElement root = xmlDocument.getRoot()) {
    if (svgpp::policy::xml::element_iterator<XMLElement>::get_child_elements(root) == NULL) {
      throw std::runtime_error("Empty root XML element");
    }
